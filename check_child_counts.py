    ...
    for resource_id in resources:
        ...
        if resource.json()["publish"] is True:
            if resource.status_code == 200:
                ...
                child_counts = check_child_counts(tree_info, child_counts, combined_aspace_id_clean)

new_dict = json.dumps(child_counts)
load_dict = json.loads(new_dict)
with open("data/check_child_count.csv", "w", encoding="utf8") as file:
    writer = csv.writer(file)
    for child, data in load_dict.items():  # TODO: not working - separating each character
        writer.writerow(str(child) + ', ' + str(data[0]) + ', ' + str(data[1]) + ', ' + str(data[2]) + ', ' + str(data[3]))
    file.close()
